...
// === Minimal Google → ACT Calendar copy endpoint ============================
import { google } from 'googleapis';

app.get('/api/calendar/google/copy-to-act', async (req, res) => {
  try {
    const TZ = 'America/Vancouver';
    const CAL_ID = process.env.GOOGLE_CALENDAR_ID || 'primary';
    const SHEET_ID = process.env.ACT_CAL_SHEET_ID || '';
    const SHEET_TAB = process.env.ACT_CAL_SHEET_TAB || 'Calendar-M01';
    const datesAsText = String(req.query.dates || '').toLowerCase() === 'text';
    ...
    function buildOAuth() {
      ...
      c.setCredentials(JSON.parse(process.env.GOOGLE_TOKENS_JSON || '{}'));
      return c;
    }
    ...
    function normRow(ge) {
      ...
      const startISO = isAllDay ? `${ge.start.date}T00:00:00` : (ge.start?.dateTime || '');
      const endISO   = isAllDay ? `${ge.end.date}T00:00:00`   : (ge.end?.dateTime || '');
      const sVal = datesAsText ? `'${startISO}` : startISO;
      const eVal = datesAsText ? `'${endISO}`   : endISO;
      return [
        ge.id,
        ge.summary || '(No title)',
        ge.description || '',
        ...
        ge.recurringEventId || '',
        ge.updated || '',
        JSON.stringify((ge.attendees || []).map(a => ({ email: a.email, response: a.responseStatus }))),
        ge.hangoutLink || ''
      ];
    }
    ...
    // Pull window: 7 days back → 60 forward
    let items = [];
    let pageToken;
    do {
      const { data } = await cal.events.list({
        calendarId: CAL_ID,
        timeMin: isoPlusDays(-7),
        timeMax: isoPlusDays(60),
        singleEvents: true,
        orderBy: 'startTime',
        showDeleted: false,
        maxResults: 2500,
    ...
    if (!SHEET_ID) {
      return res.json({ ok: true, pulled: confirmed.length, upserted: 0, note: 'ACT_CAL_SHEET_ID not set — nothing to copy.' });
    }

    const sheets = google.sheets({ version: 'v4', auth });

    // Dedupe by existing googleIds in column A
    const existingResp = await sheets.spreadsheets.values.get({
      spreadsheetId: SHEET_ID,
      range: `${SHEET_TAB}!A2:A`
    }).catch(() => ({ data: { values: [] } }));
    const existingIds = new Set((existingResp.data.values || []).map(r => r[0]).filter(Boolean));

    const newRows = [];
    for (const ge of confirmed) {
      if (!existingIds.has(ge.id)) newRows.push(normRow(ge));
    }

    if (newRows.length) {
      await sheets.spreadsheets.values.append({
        spreadsheetId: SHEET_ID,
        range: `${SHEET_TAB}!A2:M2`,
    ...
    res.json({ ok: true, pulled: confirmed.length, upserted: newRows.length, sheet: { id: SHEET_ID, tab: SHEET_TAB } });
  } catch (e) {
    console.error('[copy-to-act]', e?.response?.data || e);
